...
class RouteValidator {
  ...
  _getDefaultValue(parameter) {
    if (parameter.type.toLocaleLowerCase() == 'string') {
      return parameter.defaultValue;
    } else if (parameter.type.toLocaleLowerCase() == 'integer') {
      return parseInt(parameter.defaultValue);
    } else if (parameter.type.toLocaleLowerCase() == 'number') {
      return +parameter.defaultValue;
    } else if (parameter.type.toLocaleLowerCase() == 'date') {
      return parameter.defaultValue;
    }
  }
  ...
  _checkType(parameter, value) {
    if (parameter.type.toLocaleLowerCase() == 'string') {
      ...
      if (parameter.type.slice(0, -2).toLocaleLowerCase() == 'string') {
        for (let i = 0; i < value.length; ++i) {
          this._checkParameterString({ type: 'String', field: parameter.field + '[' + i + ']' }, value[i]);
        }
      }
      if (parameter.type.slice(0, -2).toLocaleLowerCase() == 'integer') {
        for (let i = 0; i < value.length; ++i) {
      ...
      if (parameter.type.slice(0, -2).toLocaleLowerCase() == 'number') {
        for (let i = 0; i < value.length; ++i) {
